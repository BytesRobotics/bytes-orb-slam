...
void System::Track(const cv::Mat &left_img, const cv::Mat &right_image, image_geometry::StereoCameraModel &stereo_camera_model,
    ...
    new_frame->wheel_odom = tf2::Transform();
    new_frame->wheel_odom.setOrigin(tf2::Vector3(wheel_odom->pose.pose.position.x,wheel_odom->pose.pose.position.y,0));
    new_frame->wheel_odom.setRotation(tf2::Quaternion(wheel_odom->pose.pose.orientation.x, wheel_odom->pose.pose.orientation.y,wheel_odom->pose.pose.orientation.z, wheel_odom->pose.pose.orientation.w));
    new_frame->wheel_odom *= base_to_camera; // Turn the wheel from odom->base_link to odom to left_camera_optical_link
    new_frame->altimeter_value = tf2::Vector3(0,0,altimeter->pose.pose.position.z);

    if(last_frame_ != nullptr){
        auto transformed_points = new_frame->transform_keypoints(last_frame_);
